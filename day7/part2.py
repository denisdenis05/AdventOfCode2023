...
class card:
    ...
    def countCards(self):
        ...
        for card in self.cards.keys():
            if card == "J":
                numberOfJokers = self.cards[card]
                maximumNumberOfCards = "J"
                for card in self.cards.keys():
                    if card != "J":
                        if maximumNumberOfCards == "J":
                            maximumNumberOfCards = card
                        else:
                            if self.cards[card] > self.cards[maximumNumberOfCards]:
                                maximumNumberOfCards = card
                            elif self.cards[card] == self.cards[maximumNumberOfCards] and card > maximumNumberOfCards:
                                maximumNumberOfCards = card
                if maximumNumberOfCards != "J":
